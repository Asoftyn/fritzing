...
def usage():
    print """
usage:
    makepanelizerxml3.py -f {order destination folder} -b {bom  name}
    creates a panelizer.xml file that can be used for further steps
...
def main():
    ...
    theFormat =  "<board name='{0}' requiredCount='{1}' maxOptionalCount='{2}' optionalPriority='{3}'/>\n"
    lines = []
    ...
    for row in rows:
        ...
        if filename:
            ...
            priority = row.get('optional_priority')
            ...
            theLine = theFormat.format(filename, required, optional, priority)
            lines.append(theLine)

        
    outfile = open(os.path.join(destFolder, "panelizer.xml"), "w")
    today = date.today()
    outfile.write("<panelizer width='550mm' height='330mm' small-width='225' small-height='310' spacing='6mm' border='0mm' prefix='{0}'>\n".format(today.strftime("%Y.%m.%d")))
    outfile.write("<paths>\n")
    productDir = "../../products"
